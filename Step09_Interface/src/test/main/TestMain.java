...
public class TestMain { //클래스의 범위를 나타내는 중괄호 "시작"
	/*
	 * 클래스 안에 무언가를 정의할 때는 클래스 안에 속해 있다는 의미에서 
	 * 들여쓰기를 한다
	 * 클래스 안에 정의할 수 있는 것은
	 * 1. 필드, static 필드
	 * 2. 메소드, static 메소드
	...
	public void test() {
		...
		// 어디에선가 이 클래스(TestMain) 으로 생성된 객체의 test() 메소드를
		// 호출하면 여기가 실행된다
	...
	public static void doSomething() {
		// 어디에선가 클래스명에 . 찍어서 호출하는 메소드 TestMain.doSomething()
		// 으로 호출할 수 있다
		System.out.println("doSomething() 메소드 호출됨");
	}
	
	//run 했을 때(application을 실행) 실행의 흐름이 시작되는 아주 특별한 main메소드
	public static void main(String[] args) {
		// application의 동작을 여기서 준비한다(지금은 단순히 학습을 위해 이상한
		// 뻘짓만 하고 있지만...)
