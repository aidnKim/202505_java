...
public class MyFrame extends JFrame {
	...
	public MyFrame() {
		 ...
		 * - java 10 에서 추가된 문법, 아직도 실무에서는 1.8버전을 쓰는 곳이 많기
		 *   때문에 주의!
		 * - 지역변수에만 사용 가능, 필드는 안됨
		 * - 인터페이스 type 추론 불가
		 * - null 로 초기화 불가
		...
		startBtn.addActionListener((e)->{
			...
			try {
				//결과적으로 메인스레드는 10초간 sleep 상태로 된다(작업 중이라고 가정)
		...
		startBtn2.addActionListener((e)->{
			// Thread 객체를 생성하면서 Runnable type 을 전달하고 해당 객체의
			// start() 메소드를 호출하면 새로운 작업 단위가 시작된다
	...
	public static void main(String[] args) {
		//메인 스레드에서 MyFrame 객체를 생성하고 스레드는 사용자의 입력을 처리하기
		//위해 어디선가 대기중...
		new MyFrame();
	}
}
