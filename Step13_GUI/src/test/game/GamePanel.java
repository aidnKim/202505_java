...
public class GamePanel extends JPanel{
	...
	public GamePanel() {
		...
		//드래곤 이미지 2개를 로딩해서 Image[] 배열에 미리 넣어두기
		...
		MouseMotionAdapter adapter=new MouseMotionAdapter() {
			@Override //마우스가 움직일 때마다 호출되는 메소드
		...
		addKeyListener(new KeyAdapter() {
			...
			public void keyPressed(KeyEvent e) {
				//만일 space 바를 누르면
				if(e.getKeyCode()==32) {
					// Missile 객체를 생성해서 현재 unit의 좌표를 담은 다음
					// List에 누적시키기
		...
		// Panel 이 10/1000 초마다 다시 그려지게 하기위한 타이머 설정
		Timer timer=new Timer(10, (e)->{
			//Panel 객체의 repaint() 메소드를 호출(결과적으로 paintComponent
			//메소드가 다시 호출된다)
			repaint();
		});
	...
	// Panel 이 초기화 될때 한번 호출되고 그다음에는 repaint() 라는 메소드가 호출될
	// 때 마다 다시 호출된다.
	@Override
	protected void paintComponent(Graphics g) {
		...
		//메소드의 매개변수에 전달되는 Graphics 객체를 Panel 에 그림을 그릴 수 
		//있는 도구이다.
		...
		g.drawString("count:"+count, 10, 40);
